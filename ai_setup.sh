...
# Unified script to set up AI model I/O and training for AIOS with LM Studio
echo "Starting AIOS AI model and training setup..."

# Ensure we're in the project directory
cd ~/aios_project || { echo "Failed to enter ~/aios_project"; exit 1; }

# Step 1: Check if QEMU is running on PID 158812, start if not
QEMU_PID=158812
...
# Step 2: Create or update ai_model.py with improved logic
echo "Creating ai_model.py with improved command format and reward logic..."
cat << 'EOF' > ai_model.py
import socket
import time
import torch
...
class AIOSController:
    ...
    def process_intent(self, intent, size_mb, free_mb):
        ...
        op, size = [max(0, int(x)) for x in output.tolist()]  # Non-negative integers
        cmd = f"alloc {size}" if op > 1 else f"free {size}"
        # Placeholder: Check memory at a fixed address until alloc is implemented
        response = self.comm.send_command("xp /4bx 0x1000")
        return cmd, response
...
# Step 3: Run the AI model and training loop
echo "Running ai_model.py..."
...
echo "Next steps: Implement 'alloc' command in AIOS core."
